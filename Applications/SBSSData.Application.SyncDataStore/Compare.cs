...
namespace SBSSData.Application.SyncDataStore
{
    public class Compare
    {
        public static void CompareGamesEX()
        {
            ...
            string elapsedTime = seconds > 0 ? $", and took {seconds / 60:#,###} minutes and {seconds:#,###} seconds"
                                             : ". The \"built\" data store was present and not rebuilt.";
            StringBuilder summary = new();
            summary.AppendLine($"Data store size: {dsSize:#,##0} bytes{elapsedTime}");

            Support.CopyDataStore(StaticConstants.DSPath, StaticConstants.CurrentDSPath, false);
            ...
            string html = LINQPad.Util.ToHtmlString(enableExpansions:true, noHeader:false, [summary, results]);
